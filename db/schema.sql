...
CREATE TABLE IF NOT EXISTS verification_records (
  ...
  created_at        DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
  valid_order_key       INT UNSIGNED GENERATED ALWAYS AS (CASE WHEN status='VALID' THEN order_id ELSE NULL END) VIRTUAL,
  valid_elder_meal_key  VARCHAR(60) GENERATED ALWAYS AS (CASE WHEN status='VALID' THEN CONCAT(elder_id,'_',meal_id) ELSE NULL END) VIRTUAL,
  ...
  UNIQUE INDEX uq_ver_valid_order (valid_order_key),
  UNIQUE INDEX uq_ver_valid_elder_meal (valid_elder_meal_key),
